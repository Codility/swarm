...
export default class Board extends React.Component {
  render() {
    ...
    for (let i=0; i < len; i += Board.ROW_SIZE) {
      ...
      while (componentsSlice.length < Board.ROW_SIZE && i + componentsSlice.length < len) {
        componentsSlice.push(<EmptyTile key={i + componentsSlice.length} x={i/Board.ROW_SIZE} y={componentsSlice.length}/>);
      }
